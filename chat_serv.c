...
int creat_room(int sock, char *buf);
...
void *client_main(void *arg)
{
   ...
   while ((nbyte = recv(cli_sock, buf, MAXLINE, 0)) > 0)
   {
      ...
      if (flag == F_Geust || flag == F_Host)
      {
         ...
         if (!chat_rooms[c_room_no].is_open)
         {
            pthread_mutex_lock(&mutx);
            removeClinetFromRoom(cli_sock);
            removeClient(cli_sock);
         ...
         for (int x = x_ul; x < x_dr; x++)
         {
            ...
            {
               chat_rooms[c_room_no].pic_array[x][y][0] = c_red;
               chat_rooms[c_room_no].pic_array[x][y][1] = c_green;
               chat_rooms[c_room_no].pic_array[x][y][2] = c_blue;
            }
         }
         sprintf(buf, "%03d %03d %05d %05d %05d %02d", coor_x, coor_y, c_red, c_green, c_blue, brush_size);
         //배열에 적용 끝부분
         for (int rm = 0; rm < chat_rooms[c_room_no].user_max; rm++)
         {
            if (chat_rooms[c_room_no].user_list_exist[rm])   //사람이 있을때만 메세지 전송
               if (chat_rooms[c_room_no].user_list[rm] != cli_sock)
                  send(chat_rooms[c_room_no].user_list[rm], buf, 29, 0);
         }
         pthread_mutex_unlock(&mutx);
      ...
      {
         pthread_mutex_lock(&mutx);
         c_room_no = creat_room(cli_sock, buf);
         chat_rooms[c_room_no].is_open = 1;
...
//방 종료 함수
int creat_room(int sock, char *buf)   //채팅방을 개설해 주는 함수
{
   int room_no = MAXLINE;
   //빈 방 구조체 탐색
   for (int i = 0; i < MAXLINE; i++)
   ...
   printf("chat_rooms[room_no].is_open = %d\n", chat_rooms[room_no].is_open);
   rooms_num++;
   //열린 방 번호 출력
