...
static void draw_brush(GtkWidget *widget, gdouble x, gdouble y)
{
	...
	sprintf(string_x, "%d", (int)x);
	sprintf(string_y, "%d", (int)y);
	sprintf(col_red, "%d", user_color.red);
	sprintf(col_green, "%d", user_color.green);
	sprintf(col_blue, "%d", user_color.blue);
	sprintf(col_pix, "%d", user_brush_size);
	...
	recv(s, bufall_recv, 40, 0);
	bufall_recv[30] = '\0';
	printf("[recv]%s\n", bufall_recv);

	ptr = strtok(bufall_recv, " ");
	coor_x = atoi(ptr);
	ptr = strtok(NULL, " ");
	coor_y = atoi(ptr);
	ptr = strtok(NULL, " ");
...
static gint button_press_event(GtkWidget *widget, GdkEventButton *event)
{
	...
	if (event->button == 1 && pixmap != NULL)
	{
		...
		if (whats_number == 0)
		{
			if ((event->x - 200) <= 0 && (event->y - 200) <= 0)
